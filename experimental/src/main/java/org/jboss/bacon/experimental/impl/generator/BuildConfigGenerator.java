...
public class BuildConfigGenerator {
    ...
    public BuildConfig processProject(Project project, FoundProject found) {
        ...
        if ("com.azure-azure-xml-1.2.0-AUTOBUILD".equals(name)) {
            log.debug("=== generating com.nimbusds-oauth2-oidc-sdk-11.23-AUTOBUILD");
        }
        // Strategy
        if (found.isManaged()) {
            BuildConfig buildConfig = copyManaged(found.getBuildConfig(), project);
            return updateExactMatch(buildConfig, project);
        } else if (found.isExactMatch()) {
            BuildConfig buildConfig = copyExisting(found.getBuildConfig(), found.getBuildConfigRevision(), project);
            return updateExactMatch(buildConfig, project);
    ...
    private BuildConfig generateNewBuildConfig(Project project, String name) {
        ...
        buildConfig.setBuildScript(generateBuildScript(project));
    ...
    private String updateBuildScript(String script, Project project) {
        String commentAutogenerated = "# This build configuration was modified by Autobuilder";
        return renderBuildScript(project, commentAutogenerated, script);
    }

    private String generateBuildScript(Project project) {
        ...
        return renderBuildScript(project, commentAutogenerated, defaultBuildCommand);
    }

    private String renderBuildScript(Project project, String commentAutogenerated, String buildCommand) {
        String taintedMessage = taintedMessage(project);
        String failCommand = "false";
        StringJoiner sj = new StringJoiner("\n");
        if (!buildCommand.contains(commentAutogenerated)) {
            sj.add(commentAutogenerated);
        }
        final String path = project.getSourceCodePath();
        if (path != null && !buildCommand.contains("cd " + path)) {
            // add `cd path` to the command right after any initial comments
            StringTokenizer st = new StringTokenizer(buildCommand, "\n");
            boolean cdAdded = false;
            while (st.hasMoreTokens()) {
                String line = st.nextToken();
                if (line.trim().startsWith("#") || cdAdded) {
                    sj.add(line);
                } else {
                    sj.add("cd " + path);
                    sj.add(line);
    ...
    private BuildConfig updateExactMatch(BuildConfig buildConfig, Project project) {
        boolean updated = updateAlignParams(buildConfig, project, true);
        if (updated || isTainted(project)) {
            buildConfig.setBuildScript(updateBuildScript(buildConfig.getBuildScript(), project));
        }
        return buildConfig;
    }
    ...
    private BuildConfig updateSimilar(BuildConfig buildConfig, Project project) {
        updateAlignParams(buildConfig, project, false);
        if (project.getSourceCodeRevision() == null) {
    ...
    private boolean updateAlignParams(BuildConfig buildConfig, Project project, boolean keepVersionOverride) {
        ...
        final String path = project.getSourceCodePath();
        if (path != null && !alignmentParameters.stream().anyMatch(s -> s.contains("--file="))) {
            alignmentParameters.add("--file=" + path + "/pom.xml");
        }
    ...
    public BuildConfig copyManaged(BuildConfiguration bc, Project project) {
        ...
        final String scmRevision = project.getSourceCodeRevision();
        if (scmRevision != null) {
            buildConfig.setScmRevision(scmRevision);
        }
        return buildConfig;
    }

    public BuildConfig copyExisting(BuildConfiguration bc, BuildConfigurationRevision bcr, Project project) {
        final String name = project.getName();
